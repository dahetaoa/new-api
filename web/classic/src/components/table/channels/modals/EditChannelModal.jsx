...
const GLOBAL_PASSTHROUGH_TYPE = 58;
const GLOBAL_PASSTHROUGH_DEFAULT_PATHS = {
...
const EditChannelModal = (props) => {
  ...
  const isGlobalPassthroughType = inputs.type === GLOBAL_PASSTHROUGH_TYPE;
    ...
    if (success) {
      ...
      if (data.setting) {
        try {
          ...
          data.global_passthrough_openai_response_path =
            parsedSettings.global_passthrough_openai_response_path || '';
          data.global_passthrough_openai_chat_path =
            parsedSettings.global_passthrough_openai_chat_path || '';
          data.global_passthrough_gemini_path =
            parsedSettings.global_passthrough_gemini_path || '';
          data.global_passthrough_claude_path =
            parsedSettings.global_passthrough_claude_path || '';
        } catch (error) {
  ...
  const submit = async () => {
    ...
    if (!Number(localInputs.type) || Number(localInputs.type) <= 0) {
      showInfo(t('请选择渠道类型'));
      return;
    }
    ...
    if (
      localInputs.type === GLOBAL_PASSTHROUGH_TYPE &&
      (!localInputs.base_url || localInputs.base_url.trim() === '')
    ) {
      showInfo(t('请输入API地址！'));
      return;
    }
    const isGlobalPassthrough = localInputs.type === GLOBAL_PASSTHROUGH_TYPE;
    ...
    const normalizedGlobalPaths = {
      global_passthrough_openai_response_path: isGlobalPassthrough
        ? normalizeRelativePathInput(
            localInputs.global_passthrough_openai_response_path,
          )
        : '',
      global_passthrough_openai_chat_path: isGlobalPassthrough
        ? normalizeRelativePathInput(
            localInputs.global_passthrough_openai_chat_path,
          )
        : '',
      global_passthrough_gemini_path: isGlobalPassthrough
        ? normalizeRelativePathInput(localInputs.global_passthrough_gemini_path)
        : '',
      global_passthrough_claude_path: isGlobalPassthrough
        ? normalizeRelativePathInput(localInputs.global_passthrough_claude_path)
        : '',
    };
    Object.assign(localInputs, normalizedGlobalPaths);

    // 生成渠道额外设置JSON
    const channelExtraSettings = {
      force_format: isGlobalPassthrough
        ? false
        : localInputs.force_format || false,
      thinking_to_content: isGlobalPassthrough
        ? false
        : localInputs.thinking_to_content || false,
      proxy: isGlobalPassthrough ? '' : localInputs.proxy || '',
      pass_through_body_enabled: isGlobalPassthrough
        ? false
        : localInputs.pass_through_body_enabled || false,
      system_prompt: isGlobalPassthrough ? '' : localInputs.system_prompt || '',
      system_prompt_override: isGlobalPassthrough
        ? false
        : localInputs.system_prompt_override || false,
      ...normalizedGlobalPaths,
    };
    ...
    if (isGlobalPassthrough) {
      ...
      localInputs.proxy = '';
      localInputs.pass_through_body_enabled = false;
      localInputs.system_prompt = '';
      localInputs.system_prompt_override = false;
      localInputs.allow_service_tier = false;
      localInputs.disable_store = false;
      localInputs.allow_safety_identifier = false;
      localInputs.allow_include_obfuscation = false;
      localInputs.allow_inference_geo = false;
      localInputs.allow_speed = false;
      localInputs.claude_beta_query = false;
    }
    ...
    } else if ('openrouter_enterprise' in settings) {
    ...
    } else if ('aws_key_type' in settings) {
      delete settings.aws_key_type;
    }
                          ...
                          {isGlobalPassthroughType && (
                            <>
                              <Banner
                                type='info'
                                description={t(
                                  '命中该渠道后会按客户端请求格式直接透传到上游；下方四个路径只填写公共 URL 之后的相对路径，留空时使用标准格式。',
                                )}
                                className='!rounded-lg'
                              />
                              <div>
                                <Form.Input
                                  field='global_passthrough_openai_response_path'
                                  label='OpenAI_Response'
                                  placeholder={
                              ...
                              <div>
                                <Form.Input
                                  field='global_passthrough_openai_chat_path'
                                  label='OpenAI_Chat'
                                  placeholder={
                              ...
                              <div>
                                <Form.Input
                                  field='global_passthrough_gemini_path'
                                  label='Gemini'
                                  placeholder={
                                  ...
                                  showClear
                                  extraText={t(
                                    '支持变量 {model} 和 {action}；默认会自动拼成 Gemini 原生 generateContent / streamGenerateContent 路径',
                                  )}
                                />
                              </div>
                              <div>
                                <Form.Input
                                  field='global_passthrough_claude_path'
                                  label='Claude'
                                  placeholder={
